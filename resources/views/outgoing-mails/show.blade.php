                        ...
                        <div x-show="open"
                            ...
                            @foreach ($availableStatuses as $statusCode => $statusDesc)
                                <button type="button"
                                    ...
                                    class="w-full text-left px-2.5 py-1.5 text-xs rounded-xl hover:bg-slate-100/80 flex items-center justify-between transition-all font-medium text-slate-800 cursor-pointer {{ $outgoingMail->status === $statusCode ? 'bg-slate-100/90 font-bold text-slate-900 shadow-2xs' : '' }}">
