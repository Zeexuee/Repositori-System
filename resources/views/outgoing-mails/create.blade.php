...
@section('content')
    <div class="pb-4 border-b border-gray-200">
        <h1 class="text-xl font-bold text-gray-900">Buat Draf Surat Keluar</h1>
        <p class="text-sm text-gray-600 mt-1">Lengkapi informasi dasar draf surat keluar (Nomor surat akan digenerate otomatis oleh sistem).</p>
    </div>
    ...
    <form action="{{ route('outgoing-mails.store') }}" method="POST" enctype="multipart/form-data" class="mt-6 space-y-6" x-data="{ loading: false }" @submit="loading = true">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="recipient" id="recipient" value="{{ old('recipient') }}" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-900 focus:border-blue-900 text-sm">
                @error('recipient')
                    <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div>
            <label for="file" class="block text-sm font-semibold text-gray-900 mb-1">Upload Berkas Draf PDF (Opsional pada saat Draf)</label>
