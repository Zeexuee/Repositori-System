...
<body
    class="h-full font-sans text-slate-100 antialiased ambient-bg flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8 glass-card p-8 sm:p-10 rounded-3xl shadow-2xl relative overflow-hidden">
        ...
        <div class="absolute -top-24 -right-24 w-48 h-48 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none">
        </div>
        ...
        <div>
            <div
                class="w-14 h-14 bg-gradient-to-tr from-slate-800 to-indigo-900 text-indigo-300 rounded-2xl font-bold text-2xl flex items-center justify-center mx-auto mb-4 shadow-lg border border-slate-700">
                CS
            </div>
        ...
        @if (session('error'))
            <div
                class="p-3.5 bg-rose-500/10 border border-rose-500/30 text-rose-300 rounded-xl text-xs font-semibold backdrop-blur-md">
                {{ session('error') }}
            </div>
        @endif
        ...
        @if (session('success'))
            <div
                class="p-3.5 bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 rounded-xl text-xs font-semibold backdrop-blur-md">
                {{ session('success') }}
            </div>
        @endif

        <form x-data="{ loading: false }" @submit="loading = true" action="{{ route('login') }}" method="POST"
            class="mt-6 space-y-4">
            @csrf
            <div>
                <label for="email" class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1.5">Email
                    Pengguna</label>
            ...
            <div>
                <label for="password"
                    class="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1.5">Kata Sandi</label>
        ...
        <div class="pt-5 border-t border-slate-700/60">
            <span class="block text-xs font-bold text-slate-400 uppercase text-center mb-3 tracking-wider">Login Cepat
                Peran Demo (Simulasi)</span>
            <div class="grid grid-cols-2 gap-3">
                <form action="{{ route('quick-login', ['email' => 'staf@sekretariat.corp']) }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="w-full py-2.5 px-3.5 text-xs font-medium text-slate-200 bg-slate-900/80 hover:bg-slate-800 border border-slate-700 rounded-xl text-left flex flex-col transition-all shadow-2xs hover:shadow-xs">
