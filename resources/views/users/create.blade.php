...
@section('content')
    <div class="flex items-center justify-between pb-4 border-b border-slate-200/80 mb-6">
        ...
        <a href="{{ route('users.index') }}"
            class="px-4 py-2 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold rounded-xl transition-all border border-slate-200 shadow-xs">
    ...
    <div class="max-w-2xl mx-auto bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-2xs">
        <form action="{{ route('users.store') }}" method="POST" class="space-y-4" x-data="{ loading: false }"
            ...
            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-slate-100">
                <a href="{{ route('users.index') }}"
                    class="px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-100 rounded-xl transition-all">
                    Batal
                </a>
