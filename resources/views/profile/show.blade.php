...
@section('content')
    <div class="pb-4 border-b border-slate-200/80 flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500 mt-1">Kelola data informasi akun, perbarui nama staff, dan ganti kata sandi keamanan.</p>
        </div>
    </div>
    ...
    <div class="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-2xs space-y-5">
                <div class="flex items-center space-x-4 pb-4 border-b border-slate-100">
                    <div class="w-14 h-14 bg-slate-900 text-white rounded-2xl flex items-center justify-center font-bold text-xl shadow-md border border-slate-700 flex-shrink-0">
                        {{ strtoupper(substr($user->name, 0, 2)) }}
                    </div>
                    <div class="min-w-0">
                        <h2 class="text-base font-bold text-slate-900 truncate">{{ $user->name }}</h2>
                        <p class="text-xs text-slate-500 truncate">{{ $user->email }}</p>
                        <span class="inline-flex items-center mt-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-900/5 text-slate-800 border border-slate-300">
                ...
                <div class="space-y-3.5 text-xs">
                    <div class="p-3 bg-slate-50 rounded-xl border border-slate-200/70">
                        <span class="block text-slate-500 text-[10px] font-bold uppercase tracking-wider">Nama Lengkap</span>
                        <span class="font-semibold text-slate-900 text-sm block mt-0.5">{{ $user->name }}</span>
                    </div>

                    <div class="p-3 bg-slate-50 rounded-xl border border-slate-200/70">
                        <span class="block text-slate-500 text-[10px] font-bold uppercase tracking-wider">Alamat Email</span>
                        <span class="font-semibold text-slate-900 text-sm block mt-0.5">{{ $user->email }}</span>
                    </div>

                    <div class="p-3 bg-slate-50 rounded-xl border border-slate-200/70">
                        <span class="block text-slate-500 text-[10px] font-bold uppercase tracking-wider">Peran Sistem (Role)</span>
                        <span class="font-semibold text-slate-900 text-sm block mt-0.5">{{ $user->getRoleNames()->first() ?? 'Staff' }}</span>
                    </div>
                    ...
                    <div class="p-3 bg-slate-50 rounded-xl border border-slate-200/70">
                        <span class="block text-slate-500 text-[10px] font-bold uppercase tracking-wider">Tanggal Dibuat</span>
                        <span class="font-medium text-slate-800 block mt-0.5">{{ $user->created_at?->translatedFormat('d F Y, H:i') ?? '-' }}</span>
                    </div>

                    <div class="p-3 bg-slate-50 rounded-xl border border-slate-200/70">
                        <span class="block text-slate-500 text-[10px] font-bold uppercase tracking-wider">Alamat IP Terkini</span>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-2xs space-y-5">
                ...
                <form action="{{ route('profile.update-name') }}" method="POST" class="space-y-4" x-data="{ loading: false }" @submit="loading = true">
                    @csrf
                    @method('PUT')
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-2xs space-y-5">
                ...
                <form action="{{ route('profile.update-password') }}" method="POST" class="space-y-4" x-data="{ loading: false }" @submit="loading = true">
                    @csrf
                    @method('PUT')
